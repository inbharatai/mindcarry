    ...
    page = (
      <CreateLearner
        ...
        onCreate={async (payload) => {
          ...
          if (learner) {
            setMessage(`${learner.preferredName}'s encrypted Learner Memory and private folders were created automatically.`);
            await refresh();
            setScreen('home');
          }
        }}
    ...
    <div className="app-shell">
      <header className="topbar">
        ...
        <div className="privacy-pill">
          <span className="privacy-dot" /> Encrypted local vault
        </div>
      </header>
      {(message || error) && (
      ...
      <main>{page}</main>
      <footer>
        <span>MindCarry alpha · Learner folders are created and encrypted automatically.</span>
  ...
  return (
    <section className="home-grid">
      <div className="hero-card">
        ...
        <p>
          MindCarry teaches foundational maths while building a family-controlled Learner Memory that can move
          between supported installations as an encrypted <code>.childmind</code> file.
        </p>
        ...
        <div className="truth-row">
          <div><strong>Automatic vault</strong><span>MindCarry creates all private folders. Parents do not manage files.</span></div>
          <div><strong>Model independent</strong><span>Gemini is optional intelligence, not the memory owner.</span></div>
          <div><strong>Parent controlled</strong><span>Camera and retention choices are explicit and off by default.</span></div>
        </div>
      </div>
      <aside className="side-card">
        ...
        {learners.length ? (
          <div className="learner-list">
            {learners.map((learner) => (
              <button className="learner-row" key={learner.learnerId} onClick={() => onSelect(learner)}>
                ...
                <span>
                  <strong>{learner.preferredName}</strong>
                  <small>{learnerAge(learner)} · Updated {when(learner.updatedAt)}</small>
                </span>
          ...
          <div className="empty-state">
            ...
            <p>Each child receives a separate encrypted memory protected by a parent passphrase.</p>
          </div>
        )}
  ...
  function submit(event: FormEvent) {
    ...
    void onCreate({
      ...
      consent: {
        ...
        localBehaviourAnalysisAllowed: camera,
      },
    });
  }
  ...
  return (
    <section className="form-card narrow-card">
      <div className="section-heading">
        ...
        <div><span className="eyebrow">PARENT SETUP</span><h1>Create a private Learner Memory</h1></div>
      </div>
      <form onSubmit={submit}>
        ...
        <div className="consent-panel">
          ...
          <p>MindCarry creates the learner folder, database, backup folders and session folders automatically inside its private app vault.</p>
          <div className="locked-choice">No parent needs to create, name or connect a folder manually.</div>
        </div>
        <div className="consent-panel">
          ...
          <label className="toggle-row">
            <span>Allow local camera observation</span>
            <input type="checkbox" checked={camera} onChange={(event) => setCamera(event.target.checked)} />
            <span className="toggle" />
          </label>
          <div className="locked-choice">Raw audio and raw video storage remain disabled.</div>
        </div>
        {!secureStorageAvailable && (
          <div className="inline-error">Secure operating-system storage is unavailable. MindCarry will not create an insecure learner catalogue.</div>
        )}
        <div className="field-grid">
          <label>
            Parent passphrase
            <input required minLength={12} maxLength={256} type="password" value={passphrase} onChange={(event) => setPassphrase(event.target.value)} />
            <small>Use at least 12 characters. This passphrase protects the portable learner database.</small>
          </label>
          <label>
            Confirm passphrase
            <input required minLength={12} maxLength={256} type="password" value={confirm} onChange={(event) => setConfirm(event.target.value)} />
            <small className={confirm && confirm !== passphrase ? 'field-error' : ''}>
              {confirm && confirm !== passphrase ? 'Passphrases do not match.' : 'MindCarry cannot recover a forgotten passphrase.'}
            </small>
          </label>
        </div>
        <div className="form-actions">
          <button type="button" className="secondary" onClick={onCancel}>Cancel</button>
          <button className="primary" disabled={busy || !secureStorageAvailable || passphrase !== confirm || passphrase.length < 12}>
            {busy ? 'Encrypting…' : 'Create Learner Memory'}
          </button>
        </div>
      </form>
    </section>
...
function Unlock({ learner, busy, onBack, onUnlock }: {
  ...
  return (
    <section className="unlock-card narrow-card">
      ...
      <p>Enter the original parent passphrase to decrypt this private record.</p>
...
function DashboardPage({ learner, dashboard, status, onLesson, onExport, onSettings, onLock }: {
  learner: LearnerManifest;
  dashboard: Dashboard;
  status: AppStatus | null;
  onLesson: () => void;
    ...
    <section className="dashboard-grid">
      <div className="dashboard-main">
        <div className="welcome-banner">
          <div>
            <span className="eyebrow">LEARNER HOME</span>
            <h1>Ready for the next small win, {learner.preferredName}?</h1>
            <p>MindCarry will use previous evidence and check independent understanding.</p>
          </div>
          <button className="primary large" onClick={onLesson}>Start maths lesson →</button>
        </div>
        <div className="metrics-row">
          <div className="metric-card">
            <ProgressRing value={Number(skill?.mastery || 0)} label="Mastery" />
            <div><span>Current skill</span><strong>{String(skill?.name || 'Addition within 20')}</strong><small>{String(skill?.status || 'introduced')}</small></div>
          </div>
          <div className="metric-card">
            <div className="metric-icon">🧩</div>
            <div><span>Personalisation</span><strong>{dashboard.memories.length} useful memories</strong><small>Only structured observations are kept.</small></div>
          </div>
          <div className="metric-card">
            <div className="metric-icon">🔐</div>
            <div><span>Storage</span><strong>Encrypted automatically</strong><small>Inside the local MindCarry vault.</small></div>
          </div>
        </div>
        <div className="two-column">
          <div className="panel">
            <div className="panel-header"><div><span className="eyebrow">LEARNER MEMORY</span><h2>What MindCarry remembers</h2></div></div>
              ...
              <div className="memory-list">
                {dashboard.memories.map((memory) => (
                  <div className="memory-item" key={memory.memory_id}>
                    ...
                    <p>{memory.content}</p>
                    <small>{Math.round(Number(memory.confidence) * 100)}% confidence · {Number(memory.evidence_count || 1)} evidence point(s)</small>
                  </div>
                ))}
              </div>
              ...
              <div className="session-list">
                {dashboard.recentSessions.map((session) => (
                  <div className="session-item" key={session.session_id}>
                    ...
                    <div><strong>{String(session.objective)}</strong><p>{String(session.summary || 'Summary pending.')}</p><small>{when(session.started_at)}</small></div>
                  </div>
                ))}
              </div>
      ...
      <aside className="dashboard-side">
        <div className="profile-panel">
          <div className="large-avatar small">{learner.preferredName[0]?.toUpperCase()}</div>
          <h2>{learner.preferredName}</h2>
          <p>Age {learner.age}</p>
          <div className="tag-row">{dashboard.profile.interests.map((interest) => <span key={interest}>{interest}</span>)}</div>
        </div>
        <div className="action-panel">
          <button onClick={onExport}>⇩ Export encrypted .childmind</button>
        ...
        <div className="status-panel">
          ...
          <span>Camera: {Number(dashboard.consent.camera_allowed) ? 'allowed' : 'off'}</span>
          <span>Raw video storage: off</span>
          <span>AI provider: {status?.provider || 'demo'}</span>
        </div>
      </aside>
    </section>
  );
  ...
  if (result) {
    return (
      <section className="completion-card narrow-card">
        ...
        <h1>Good thinking, {learner.preferredName}.</h1>
        <p>{result.explanation}</p>
        <ProgressRing value={Number(result.mastery || 0)} label="Mastery" />
        <div className="completion-note"><strong>Next time</strong><span>MindCarry will retrieve today&apos;s structured observations.</span></div>
    ...
    <section className="lesson-layout">
      <div className="lesson-stage">
        <div className="lesson-top">
          <button className="back-button" onClick={() => void cancelAndExit()}>←</button>
          <div><span className="eyebrow">FOUNDATIONAL MATHS</span><strong>Addition within 20</strong></div>
          <span className="live-badge">● Private session</span>
        </div>
        <div className="tutor-bubble"><div className="tutor-avatar">M</div><p>{message}</p></div>
        {question && (
          <div className="question-card"><span>Try this</span><h1>{question.prompt}</h1><div className="visual-aid">{question.visual}</div></div>
        )}
        <form className="answer-area" onSubmit={submit}>
          <label>
            Your answer
            <div className="voice-input">
              <input maxLength={100} value={answer} onChange={(event) => setAnswer(event.target.value)} placeholder="Say or type the answer" />
              <button
                type="button"
                className="mic"
                disabled={!microphoneEnabled}
                title={microphoneEnabled ? 'Use microphone' : 'Microphone disabled by parent'}
                onClick={() => {
                  if (!microphoneEnabled) {
                    setError('Microphone access is disabled for this learner. Type the answer instead.');
                    return;
                  }
                  stopListening.current?.();
                  stopListening.current = listenOnce(
                    ...
                    dashboard.profile.preferred_language,
                  );
                }}
              >
                🎙
          ...
          <div className="lesson-actions">
            <button
              ...
              onClick={() => {
                setUsedHint(true);
                setMessage('Start with the larger number, then count on one step at a time.');
                speak('Start with the larger number, then count on one step at a time.');
              }}
            >
              Show me another way
            </button>
            <button className="primary" disabled={busy || !answer.trim()}>{busy ? 'Thinking…' : 'Check my thinking →'}</button>
          </div>
        </form>
      ...
      <aside className="lesson-observer">
        ...
        <div className="observer-card">
          <span className="eyebrow">OBSERVABLE CUES</span>
          <h3>Local movement signal</h3>
          <strong>{cameraEnabled ? `${Math.round(movement * 100)}%` : 'Off'}</strong>
          <p>This is not an emotion, diagnosis or judgement.</p>
        </div>
        <div className="observer-card">
          <span className="eyebrow">WHAT IS SAVED</span>
          <p>Answer evidence, response time, misconception and intervention outcome. Raw video is not saved.</p>
        </div>
      </aside>
    </section>
  );
...
function Settings({ status, busy, run, onBack, onChanged }: {
  ...
  return (
    <section className="settings-card narrow-card">
      <div className="section-heading">
        <button className="back-button" onClick={onBack}>←</button>
        <div><span className="eyebrow">SETTINGS</span><h1>Gemini and encrypted local storage</h1></div>
      </div>
      <div className="settings-section">
        <h3>Current AI mode</h3>
        <div className="provider-card large">
          <span className={`provider-light ${status?.provider === 'gemini' ? 'connected' : ''}`} />
          <div>
            <strong>{status?.provider === 'gemini' ? 'Gemini connected' : 'Safe demo mode'}</strong>
            <small>{status?.provider === 'gemini' ? status.model : 'No API key is required for the deterministic demo.'}</small>
          </div>
        </div>
      </div>
      <form
        ...
        onSubmit={async (event) => {
          event.preventDefault();
          const response = await run(() => window.mindcarry.settings.setGeminiKey(apiKey));
          if (response) {
            setResult(response);
            setApiKey('');
            await onChanged('Gemini key was tested successfully and stored with operating-system encryption.');
          }
        }}
      >
        <h3>Gemini test API key</h3>
        <p>MindCarry tests the key before enabling Gemini. The key never enters a learner folder or <code>.childmind</code> export.</p>
      ...
      <div className="settings-section">
        <div className="button-row">
          <button className="secondary" onClick={async () => {
            const response = await run(() => window.mindcarry.settings.testProvider());
            if (response) setResult(response);
          }}>Test provider</button>
          <button className="danger-link" onClick={async () => {
            const response = await run(() => window.mindcarry.settings.removeGeminiKey());
            if (response) await onChanged('Gemini key removed. Demo mode restored.');
          }}>Remove Gemini key</button>
        </div>
        {result && (
          <div className={`provider-result ${result.ok ? 'ok' : 'bad'}`}>
      ...
      <div className="settings-section">
        <h3>Automatic local vault</h3>
        <p>MindCarry created and manages this folder automatically. Learner databases inside it are encrypted with the parent passphrase.</p>
        <code className="path-code">{status?.vault.root || 'Loading…'}</code>
        <div className="button-row">
          <button className="secondary" onClick={() => void run(() => window.mindcarry.app.openDataFolder())}>Open vault folder</button>
          <span>{status?.vault.ready ? 'Vault ready' : 'Vault unavailable'}</span>
        </div>
      </div>
    </section>
  );
